...
| DomainVerified | the trigger resolved from the Stage 2 open question, before task 14 | `tenant_domain` / tenant domain ID | `tenant_domain_id`, `tenant_id`, `domain` |
...
- `DomainVerified` trigger semantics are an open question in the Stage 2 plan (registered, first confirmed by the ask endpoint, or a dedicated verification flow); it must be resolved before task 14 attaches the producer. The payload contract fixed by the Stage 2 plan stays as written regardless of the answer.
