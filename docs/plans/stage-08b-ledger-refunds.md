...
- Order state machine extensions: `paid` to `partially_refunded` and `paid` to `refunded` (system-design 7.1), plus the follow-on transitions from `partially_refunded` (see Risks; the 7.1 diagram omits them and needs a design update in the same change). The transition Actions and their state-machine table test live in the Orders context; Payments invokes them (system-design 3.1 boundary rule: a context never writes another context's tables).
...
11. Orders context transition Actions for the new arcs out of `partially_refunded`, extending the Stage 7 state-machine table test, with the system-design 7.1 diagram amendment in the same change. Scope `orders`. Mergeable independently of 9.
