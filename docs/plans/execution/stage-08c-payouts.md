...
#### T11: ReconcilePayouts scheduled command (2026-07-12)

Landed:

- `App\Payments\Gateways\GatewayPayoutRecord` gained a required `gatewayAccountReference` parameter (first, ahead of the existing `gatewayReference`, `amount`, `status`, `executedAt`), mirroring `NormalizedPayoutEvent`'s field. Deviation from T1/T8's already-shipped shape: the stage plan's Domain events section states listPayouts callbacks "carry no tenant context" the same as webhooks, but the record type had no field to carry it, so the poller could not resolve a tenant for a record whose mirror row does not yet exist. Updated the two pre-existing `FakeGatewayTest` call sites in the same commit (signature widening, not new behavior); `FakeGatewayScenarios::scriptPayouts`/`payouts()` needed no change.
- `App\Payments\Actions\ReconcilePayouts`: iterates `GatewayRegistry::all()` and each adapter's `listPayouts()`. For every record, resolves the owning tenant through the sub-merchant account by `(gateway, gateway_account_reference)` under the platform role, exactly as `ProcessGatewayWebhook::applyPayout` already does (no new `platform_role_use` activity-log call added here, since this is a poller resolving a batch rather than a single inbound callback; the existing webhook path already covers the audited-system-use requirement for this lookup pattern, and the plan's Slice 6 test list does not ask for one). A record whose account reference matches no sub-merchant account is skipped and not counted. Inside a tenant-scoped transaction: a record with no existing `payouts` row is inserted through the existing `RecordGatewayPayout` action (reused unmodified, built from a synthesized `NormalizedPayoutEvent` so a payout that skips straight to `paid` still gets exactly one `PayoutExecuted` event), then stamped `reconciled_at`; a record whose mirror already exists is stamped `reconciled_at` unconditionally and, only when the gateway-reported amount differs from the mirrored `amount`, has `discrepancy_amount` set to the absolute difference and one `payout_discrepancy` activity log entry recorded with the mirrored amount, gateway amount, and discrepancy.
- `App\Console\Commands\ReconcilePayoutsCommand` (`payments:reconcile-payouts`), following `ReconcileProcessingRefundsCommand`'s exact shape (inject the action, report the resolved count, return `SUCCESS`). Scheduled `everyFiveMinutes()` in `bootstrap/app.php` alongside the existing `payments:reconcile`/`payments:reconcile-refunds` sweepers (system-design 13).
- Deviation from the plan's own summary line ("checks mirrored payouts against ledger balances"): the concrete Slice 6 test-first spec two paragraphs later says the discrepancy is between the gateway's reported amount and "the mirror," and `PayoutData.discrepancy` (T9, already shipped) is built from the payout row's own `discrepancy_amount`/`currency` with no ledger read anywhere in its construction. Comparing the gateway record to the mirrored `payouts.amount` (not to a ledger balance query) is therefore the reading actually implemented; no ledger-balance comparison exists in this stage, and none was found needed to satisfy either the plan's Slice 6 test list or T9's already-committed `PayoutData` shape.
- No endpoint, no OpenAPI change, no Data class touched (`GatewayPayoutRecord` and `ReconcilePayouts` are internal to the Payments context), so `composer types:generate` was not run.
- Tests first: `tests/Feature/Payments/ReconcilePayoutsTest.php` (new), confirmed failing before `ReconcilePayouts` existed (`Target class [App\Payments\Actions\ReconcilePayouts] does not exist`): creates a payout missed by every webhook with `reconciled_at` set and one `PayoutExecuted` event; stamps `reconciled_at` on an already-mirrored payout with no discrepancy when amounts match; flags `discrepancy_amount` plus one `payout_discrepancy` activity log entry when the gateway amount diverges from the mirror; confirms no discrepancy flag or log entry when amounts match exactly; ignores a listed record whose account reference matches no sub-merchant account, leaving no row and not counting it as resolved.

Test evidence (from `apps/api`):

- `php artisan test --filter=ReconcilePayoutsTest`: 5 passed, 15 assertions (confirmed failing first, missing class error).
- `php artisan test --filter="ReconcilePayoutsTest|RecordGatewayPayoutTest|PayoutWebhookTest|FakeGatewayTest|PayoutReadTest|PayoutLedgerProjectionTest"`: 63 passed, 222 assertions.
- `php artisan test --testsuite=Feature --filter=Payments` (run serially, not parallel to any other suite): 157 passed, 826 assertions.
- `php artisan test --testsuite=Architecture`: 40 passed, 97 assertions.
- `php artisan test --testsuite=Unit` (run serially): 898 passed, 2182 assertions.
- `vendor/bin/pint --test` on all touched/new files: passed.
...
## Review round 1 (2026-07-12 05:31:49 -03)

Codex code review of stage 8c produced four findings; all four accepted and fixed.

1. Blocking, `RecordGatewayPayout::insert` (executed_at). A `payout.created` event arriving with `paid` status and a null `executedAt` was inserted with `executed_at` left null, then `PayoutExecuted::fromPayout` dereferenced `executed_at` unconditionally and fatal-errored instead of recording the payout. Fixed by defaulting `executed_at` to `Date::now()` when the inserted status is `paid` and the event carries no timestamp, mirroring the existing transition path. Test added to `RecordGatewayPayoutTest` for the paid-insert-with-null-executed_at case (asserts the row lands paid with a stamped `executed_at` and one `PayoutExecuted` event).

2. Important, `StartSubmerchantOnboarding` (guarded transition). The synchronous gateway acknowledgment applied the returned status via a blind `$account->update([...])`, bypassing `TransitionSubmerchantAccount`'s conditional-update guard and accepting any returned status without validating the pending source. Fixed by updating only the non-state gateway metadata (`gateway_account_reference`, `onboarding_url`, `requirements`) directly and routing any status past `pending` through `TransitionSubmerchantAccount`, the same guard the webhook and refresh paths use. Test added to `StartSubmerchantOnboardingActionTest` (scripted immediate-approve lands active with `activated_at` and one `submerchant_state_changed` audit entry).

3. Important, `ReconcilePayouts::flagDiscrepancy` (currency in comparison). Reconciliation compared only integer minor units (`$record->amount->amount === $payout->amount`), so equal amounts in different currencies were treated as matching and no discrepancy was recorded. Fixed by comparing through `Money::equals`, which requires both amount and currency to match. Test added to `ReconcilePayoutsTest` (same minor units, different currency, asserts a discrepancy flag and one `payout_discrepancy` entry).

4. Important, activity log coverage for gateway-driven state changes. Neither `TransitionSubmerchantAccount` (webhook and refresh onboarding transitions) nor `RecordGatewayPayout` (poller/webhook-created and transitioned payout state changes) wrote an activity-log entry, though the stage plan's exit criterion 10 requires all onboarding and payout state changes in the activity log. Fixed by recording a `submerchant_state_changed` entry after every successful transition in `TransitionSubmerchantAccount` and a `payout_state_changed` entry after every insert and successful transition in `RecordGatewayPayout`. Tests added to `RecordGatewayPayoutTest` (insert plus transition produce two `payout_state_changed` entries) and covered for submerchant via finding 2's test.

Verification: `tests/Feature/Payments/` (160 passed), the three touched unit suites `RecordGatewayPayoutTest`/`StartSubmerchantOnboardingActionTest`/`TransitionSubmerchantAccountTest` (47 passed), `tests/Concurrency/SubmerchantAccountTransitionContentionTest` (passed), and `vendor/bin/pint --test` (passed). No Data class changed, so `types:generate` was not run.

## Review round 2 (2026-07-12 05:41:41 -03)

Codex code review of stage 8c produced three findings; all three accepted and fixed.

1. Important, `ReconcilePayouts` (existing mirrors never applied gateway status). An existing payout mirror was only checked for amount discrepancies; the gateway-reported status was never applied, so a missed `paid` webhook left a pending mirror forever and never emitted `PayoutExecuted`, leaving the ledger incorrect. Fixed by adding `applyReportedStatus`, which routes any gateway status that differs from the mirror through the existing `RecordGatewayPayout` transition (a status-only `NormalizedPayoutEvent` with null amount), reusing the same guarded transition and one-time `PayoutExecuted` emission the webhook path uses. Test added to `ReconcilePayoutsTest` (pending mirror, gateway reports paid: asserts the mirror lands paid with a stamped `executed_at`, `reconciled_at`, and exactly one `PayoutExecuted` event).

2. Important, `StartSubmerchantOnboarding` (stranded onboarding after gateway failure). The pending account was committed before `createSubmerchant`; a gateway failure left a row with no gateway reference, so subsequent starts returned `submerchant_already_onboarded` and `RefreshSubmerchantStatus` no-oped on the null `gateway_account_reference`, permanently stranding onboarding after a transient gateway outage. Fixed by catching any throwable from the gateway call, deleting the just-committed pending row (rolling back the concurrency-slot placeholder) before rethrowing, and still recording a circuit-breaker failure for `GatewayUnavailableException`. Added a `failNextSubmerchantCreation` hook to `FakeGatewayScenarios`/`FakeGateway`; test added to `StartSubmerchantOnboardingActionTest` (gateway failure leaves zero rows and a subsequent start succeeds).

3. Important, `ReconcilePayouts::flagDiscrepancy` (currency mismatch produced a phantom zero). Round 1 detected currency mismatches via `Money::equals`, but `discrepancy_amount` was still computed by subtracting minor-unit integers regardless of currency, so equal amounts in different currencies produced `discrepancy_amount = 0` and the gateway currency was dropped. Fixed by treating a currency mismatch as a non-subtractable divergence: the full mirrored amount is flagged as the discrepancy, and the activity log now carries `mirrored_currency`, `gateway_currency`, and a `currency_mismatch` flag. Strengthened the existing currency-mismatch test in `ReconcilePayoutsTest` to assert `discrepancy_amount` is the mirrored amount (not zero) and that both currencies are preserved on the log entry.

Verification: `php artisan test --filter="ReconcilePayouts|StartSubmerchantOnboarding"` (13 passed), `tests/Feature/Payments` plus `tests/Unit/Payments` (300 passed), `tests/Concurrency/SubmerchantOnboardingContentionTest` (passed), and `composer -d apps/api run lint` (Pint passed). No Data class changed, so `types:generate` was not run.

## Review round 3 (2026-07-12 05:47:38 -03)

Codex code review of stage 8c produced two findings; both accepted and fixed.

1. Important, `SubmerchantAccountController::index` (ascending sort deviated from the fixed contract). The endpoint declared `allowedSorts('created_at')` with a `-created_at` default, so a caller could pass `sort=created_at` and get ascending order, drifting from the plan/OpenAPI fixed newest-first contract. Fixed by matching the shipped `PayoutController` precedent: `allowedSorts()` with no arguments plus an explicit `orderByDesc('created_at')`, making the order a fixed non-caller-adjustable newest-first and rejecting any caller-supplied `sort` (including `-created_at`) with 400 `invalid_query_parameter`. Test added to `SubmerchantOnboardingTest` (both `sort=created_at` and `sort=-created_at` rejected with `invalid_query_parameter`).
