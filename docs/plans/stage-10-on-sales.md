...
- `onsale:{tenant_id}:{event_id}:budget`: the event's admission token bucket (a hash of `tokens` and `updated_at`), refilled and charged atomically by the gatekeeper. Tokens accrue with elapsed time at `admission_rate_per_minute` measured on the application clock, and the bucket holds at most one tick's worth of them, so a tick releases only what has accrued since the last one. This is what makes the waiting room a shaper rather than a counter: a bucket that could hold a whole minute's rate would let the first tick of each minute dump the entire budget into checkout and then idle, which is the thundering herd the waiting room exists to prevent. The bucket starts empty and accrual is the only thing that adds to it, so admissions in any 60-second window still cannot exceed `admission_rate_per_minute`, and an idle event banks no burst to spend later.
...
Admission and dequeue run as Lua scripts so that refilling the bucket, popping an entrant from `waiting`, writing it to `admitted`, and charging the bucket are one atomic step; two racing gatekeeper runs cannot admit the same entrant twice or between them release more than the tick's allowance. This is the queue-side equivalent of the affected-row-count guard.
...
- Concurrency (first): two gatekeeper processes racing on one queue admit at most one tick's allowance with no entrant admitted twice (Lua atomicity proven by affected-entrant accounting).
- Feature (first): with rate R and W waiting entrants, one gatekeeper tick admits min(one tick's allowance of R, W) in arrival order, and a full minute of ticks admits R spread evenly across them rather than all at once on the first tick (fake clock, all six ticks asserted); the poll endpoint then returns `admitted` with a token and expiry; an entrant polling after token expiry (fake clock) gets 404; per-event rate honored across two flagged events of different tenants in one tick (cross-tenant config read via the section 4.3 role, following the Stage 4 worker precedent). The bucket's tick allowance and the gatekeeper's own scheduler cadence are asserted to be in step, since nothing else would catch them drifting apart.
- Unit (first): token payload and HMAC round-trip; a signing secret that is unset or blank fails closed on both issue and verify (never an empty-key HMAC); key-ID rotation (old key verifies until retired, unknown key ID rejected); expiry checked against the injected clock; token bucket arithmetic (accrual with elapsed time, the one-tick cap, no burst banked over an idle gap, and the per-minute ceiling holding under ticks denser than the cadence).
...
5. The gatekeeper admits in arrival order at the configured per-event rate, spreads that rate evenly across the minute's ticks rather than releasing it in one burst, never exceeds the per-minute rate (nor, under racing gatekeeper processes, a single tick's allowance), and admits across events of different tenants in one tick.
...
- Gatekeeper cadence. The gatekeeper needs sub-minute ticks to make a per-minute admission rate feel smooth, and the admission budget has to be a token bucket that accrues with elapsed time for those ticks to mean anything: a budget that simply resets on the minute boundary would let the first tick of each minute spend all of it, which is the burst the sub-minute cadence was introduced to avoid. The bucket's capacity is therefore one tick's worth of the rate, which couples it to the scheduler's cadence; the two live in `config/onsale.php` and `bootstrap/app.php` respectively, and a test asserts they never drift, because a capacity narrower than the real tick silently under-admits and a wider one silently restores the burst. The scheduler's sub-minute support in the pinned Laravel version must be verified against current framework documentation at implementation time; the fallback is a supervised long-running command in the scheduler container (section 16.3 already runs dedicated scheduler and worker processes).
