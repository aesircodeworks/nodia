...
- Security sweep: meta-tests asserting isolation suite coverage for every tenant-scoped endpoint and contract coverage for every route; an authorization matrix completeness check asserting every mutating route maps to a capability; webhook signature negative tests re-run across all registered gateways; secret handling review with a repo secret scan wired into CI and an architecture test forbidding `env()` outside `config/` (system-design 14.1, 14.2).
...
- Secret handling: a repo secret scanner (tool selected at slice start; gitleaks is the default candidate) runs in CI and fails on findings; an architecture test forbids `env()` calls outside `config/`; a test asserts `.env.example` files contain placeholders only; the review's manual findings are fixed in-stage, not recorded as TODOs.
...
16. Secret scanner in CI, `env()` architecture test, `.env.example` assertions; fix anything found. Scope: `ci`, fixes under their owning scopes.
...
8. The authorization matrix test denies every capability a role lacks across all routes; webhook signature negatives reject before persistence for every registered gateway; the secret scan gate is required in CI; no `env()` outside `config/`.
...
10. `docs/load-targets.md` records targets, tool, profiles, and measured results for hold creation, payment initiation, and waiting room admission, each meeting its target with headroom stated; post-run probes show zero oversell and zero duplicate payment effects.
