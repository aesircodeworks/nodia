...
Money is a schema convention this stage codifies without creating columns: every monetary value is an integer minor unit paired with a `currency` column on the same row, never floats, never an amount without a currency, named `*_amount` except on the tables data-conventions exempts because their row is itself a single monetary fact, which carry bare `amount` (data-conventions Money; ADR 018). The Eloquent cast tests use an in-test probe table with `price_amount bigint` and `currency char(3)` to prove round-tripping.
