...
- The order state machine statuses (`pending`, `awaiting_payment`, `paid`, `expired`, `failed`, `canceled`, `partially_refunded`, `refunded`) with every transition implemented as a conditional UPDATE checked by affected-row count (system-design 7.1, data-conventions). Transition Actions for `awaiting_payment`, `paid`, `expired`, and `failed` ship and are fully tested here; Stage 8a wires payments to them. The refund statuses exist in the enum from day one, but every refund transition (the arcs into and out of `partially_refunded` and `refunded`) is Stage 8b's, which also owns the system-design 7.1 amendment for the arcs the diagram omits.
...
- Refund execution, commission policy, and ledger projection: Stage 8b, which also implements every refund transition (`paid` to `partially_refunded`/`refunded` and the follow-on arcs) together with the system-design 7.1 amendment for the arcs the diagram omits. This stage defines the `TicketRefunded`/`TicketCanceled` event classes so the registry is complete, but ships no refund transition Actions.
...
- Stage 8b: the `OrderStatus` refund statuses, the state-machine table test it extends with the refund arcs, and `TicketRefunded`.
...
- Unit: a data-driven table over every ordered pair of non-refund states (`pending`, `awaiting_payment`, `paid`, `expired`, `failed`, `canceled`) asserting exactly the system-design 7.1 arcs among them succeed and every other pair raises with zero rows affected; the refund states are asserted unreachable in this stage (no transition Action targets them), so Stage 8b's refund arcs extend the table additively instead of rewriting frozen assertions; each transition Action is a single conditional UPDATE (asserted by affected-row count on a concurrent pre-transition).
...
Implement: `OrderStatus` enum, transition Actions (`MarkOrderAwaitingPayment`, `MarkOrderPaid` guard only, `MarkOrderExpired`, `MarkOrderFailed`, `CancelOrder`), hold release wiring, the outbox subscriber.
...
4. The state machine table test proves exactly the system-design 7.1 arcs among the non-refund states succeed and all other transitions affect zero rows and raise `invalid_order_transition`; the refund arcs are proven by Stage 8b's extension of the same table.
...
- The 7.1 refund arcs are incomplete for Stage 8b's needs: the diagram draws no `partially_refunded` to `refunded` arc (completing a refund after a partial) and no repeated `partially_refunded` to `partially_refunded` arc, yet "refunds, full and partial" needs both. Resolved by deferring every refund transition to Stage 8b: the slice 2 table test covers only the non-refund states, so 8b's arc additions and its system-design 7.1 amendment extend the test additively instead of rewriting assertions this stage froze.
