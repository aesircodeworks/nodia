...
- `tenant_id` NULL means a global template role maintained by the platform (Owner, Event Manager, Box Office, Finance, Check-in Agent); a set `tenant_id` means a tenant custom role (system-design 5.3). This is the sanctioned exception to the non-null `tenant_id` rule now recorded in data-conventions and the master plan's Stage 3 bullet: templates must be readable in every tenant context, which a sentinel-tenant row cannot provide under a single-tenant RLS predicate. See risks.
...
- `roles.tenant_id` nullable is the exception data-conventions now records (Tenancy section) and the master plan's Stage 3 bullet sanctions. Justified by system-design 5.3 (NULL means global template) and by the requirement that templates be readable under every tenant's RLS context. The mitigation is the mutation policy restricted to the current tenant plus the partial unique index, and the isolation suite's tenant-scoped-table sweep special-cases `roles` with explicit template read and write probes. The alternative, a separate `role_templates` table without `tenant_id` at the cost of a union read path, remains the fallback if the template-aware policy proves problematic in practice.
