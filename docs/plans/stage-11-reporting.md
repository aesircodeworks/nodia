...
- Stage 9: `TicketCheckedIn` and `DuplicateScanDetected` events for the attendance projection. The master plan lets Stages 9, 10, and 11 proceed independently; if this stage starts before Stage 9 lands, the attendance slice (slices 5 and 6, tasks 10 through 12) is gated on Stage 9's events and everything else proceeds.
