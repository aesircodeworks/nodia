...
const prep = await agent(
  ...
  'Return baseCommit, branch, and the ordered list of REMAINING tasks only (skip work you verified is already done). Size each task so one focused agent can take it through the full TDD loop in one session. Each task needs: id, short title, a detail paragraph citing the plan sections it covers, the tests that must be written first, and the owning bounded-context commit scope. Do not include full-suite or gate-phase verification (full test suite, Larastan, Pint across the repo, contract drift gate) in any task; the gate phase owns that. A task runs only the scoped tests covering its own changes. If the whole stage is verifiably complete, return alreadyComplete true and an empty task list.',
